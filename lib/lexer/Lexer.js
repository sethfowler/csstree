...
function matchSyntax(lexer, syntax, value) {
    ...
    if (valueHasVar.call(lexer, value)) {
        return {
            type: 'NoMatch',
            comment: 'Due to matching for value with var() is too complex those values are always valid for now'
        };
    }
    ...
    if (config) {
        if (config.generic) {
            ...
            for (var name in generic) {
                this.types[name] = this.createDescriptor(generic[name]);
                if (name === 'expression') {
        ...
        if (config.types) {
            for (var name in config.types) {
                this.types[name] = this.createDescriptor(config.types[name]);
            }
        }

        if (config.atrules) {
            for (var atruleName in config.atrules) {
                this.atrules[atruleName] = {
                    descriptors: {}
                };
                for (var name in config.atrules[atruleName].descriptors) {
                    this.atrules[atruleName].descriptors[name] = this.createDescriptor(
                        config.atrules[atruleName].descriptors[name]
                    );
                }
            }
        }
        ...
        if (config.properties) {
            for (var name in config.properties) {
                this.properties[name] = this.createDescriptor(config.properties[name]);
            }
        }
    }
};
...
Lexer.prototype = {
    ...
    lastMatchError: null,
    matchProperty: function(propertyName, value, atrule) {
        ...
        if (property.variable) {
            return {
                type: 'NoMatch',
                comment: 'Lexer matching is not applicable for custom properties'
            };
        }

        var propertySyntax = null;

        if (atrule) {
            propertySyntax = this.getAtruleProperty(atrule, property.name);
        }

        if (!propertySyntax) {
            propertySyntax = property.vendor
    ...
    getAtruleProperty: function(atrule, name) {
        return (this.atrules[atrule] && this.atrules[atrule].descriptors[name]) || null;
    },
    ...
    validate: function() {
        function validate(syntax, name, broken, descriptor) {
            ...
            if (descriptor.syntax !== null) {
                walk(descriptor.syntax, function(node) {
                    ...
                    if (!map[node.name] || validate(syntax, node.name, brokenMap, map[node.name])) {
                        broken[name] = true;
                    }
                }, this);
            }
        ...
        for (var atrule in this.atrules) {
            var atruleBrokenMap = {};

            for (var key in this.atrules[atrule].descriptors) {
                validate(this, key, atruleBrokenMap, this.atrules[atrule].descriptors[key]);
            }

            atruleBrokenMap = Object.keys(atruleBrokenMap).filter(function(name) {
                return atruleBrokenMap[name];
            });

            if (atruleBrokenMap.length) {
                brokenAtrules[atrule] = atruleBrokenMap;
            }
        }
        ...
        if (brokenTypes.length ||
            Object.keys(brokenAtrules).length ||
            brokenProperties.length) {
            return {
                properties: brokenProperties,
                atrules: brokenAtrules,
                types: brokenTypes
            };
        }
    ...
    dump: function(syntaxAsAst) {
        return {
            ...
            atrules: Object.keys(this.atrules).reduce(function(result, atrule) {
                result[atrule] = {
                    descriptors: dumpMapSyntax(this.atrules[atrule].descriptors, syntaxAsAst)
                };
                return result;
            }.bind(this), {}),
